...
for i= 1:6
    ...
    range_terminal=802+802*(i-1);
    extract_start=350;
    extract_terminal=801;
    ...
    gm_top=total_data_position(raw_data_start+802*(i-1),5);
    raw_data_terminal=length(time);
    raw_data_range=raw_data_start:raw_data_terminal;
    %to find the terminal index for fitting data
    fitted_start=find_index_extract(fitted_start,6,gm_top-pos_z_extract);
    %to find the terminal index for fitting data
    fitted_terminal=find_index_extract(fitted_terminal,4,gm_top-pos_z_extract);
