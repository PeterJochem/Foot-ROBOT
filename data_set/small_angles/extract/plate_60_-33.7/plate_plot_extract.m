...
for i= 1:1
    ...
    gm_top=total_data_position(500+802*(i-1),5);
    raw_data_terminal=length(time);
    raw_data_range=raw_data_start:raw_data_terminal;
    %to find the terminal index for fitting data
    fitted_start=find_index_extract(fitted_start,9,gm_top-pos_z_extract);
    %to find the terminal index for fitting data
    fitted_terminal=find_index_extract(fitted_terminal,3,gm_top-pos_z_extract);
