...
for k =1:11
    tdown(k)=time_settle+0.7*(k-1);
    thorizontal(k)=time_settle+intrude_period+0.7*(k-1);
end
...
for i =1:10
    subplot(4,3,i)
    start_index=find_index(1,thorizontal(i),total_data_force(:,1))+1;
    term_index=find_index(1,tdown(i+1),total_data_force(:,1));
