...
const double particle_moving = 5.0;
...
int main(int argc, char* argv[]) {
   ...
   // std::cout <<"vector size:  "<< pointsFloat3.size()<<std::endl;
    ...
    std::string mesh_filename_lid = GetChronoDataFile("granular/demo_impact/foot.obj");
...
// create a plate for simulation
  ...
  //  sys_plate.AddLink(my_Link_reffoot);
    ...
    double v_array[10] = { -1.0,-2.0,-5.0,-10.0,-20.0,-30.0,-40.0,-50.0,-100.0,-10000.0 };
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
           if (t >= particle_moving && plate_impact_state==false) {
                ...
                max_z = gran_sys.get_max_z();
                rigid_plate->SetBodyFixed(false);
                rigid_plate->SetPos_dt(ChVector<>(vxx, 0, vzz));
                rigid_plate->SetRot(Q_from_AngAxis(CH_C_PI / 6.0, VECT_Y));
                rigid_plate->SetPos(ChVector<>(0, 0, max_z+2.5*sin(CH_C_PI / 6.0)-0.2));
                plate_impact_state = true;
              //  sys_plate.AddLink(my_Link_reffoot);
            }/*
           if (t >= particle_moving  && plate_impact_state == true) {

               rigid_plate->SetPos_dt(ChVector<>(0, 0, -3.0));
               rigid_plate->SetRot(Q_from_AngAxis(0, VECT_Y));
           }*/
         /*
            if (t >= particle_moving && lid_released == false) {
                gran_sys.enableMeshCollision();              
                rigid_lid->SetBodyFixed(false);
                max_z = gran_sys.get_max_z();
                rigid_lid->SetPos(ChVector<>(0, 0, max_z + 11));
                rigid_lid->SetPos_dt(ChVector<>(0, 0, 0));
                lid_released = true;
            }
            if (t >= particle_moving && lid_released == true) {
                rigid_lid->SetRot(Q_from_AngAxis(0, VECT_Y));
            }
            if (t >= particle_moving + lid_compress) {
                rigid_lid->SetBodyFixed(true);
                rigid_lid->SetPos(ChVector<>(0, 0, 100));
            }
...
/*            if (t >= particle_moving + lid_compress + time_settle2 && plate_impact_state == true) {
           ...
           // rigid_plate->Accumulate_torque(ChVector<>(plate_force[3], plate_force[4], plate_force[5]), false);
            rigid_plate->Accumulate_force(ChVector<>(plate_force[0], 0, plate_force[2]), plate_pos, false);
            rigid_plate->Accumulate_torque(ChVector<>(0, plate_force[4], 0), false);
         ...
         //   rigid_plate->Accumulate_force(ChVector<>(plate_force[0], plate_force[1], plate_force[2]), plate_pos, false);
          //  rigid_plate->Accumulate_torque(ChVector<>(plate_force[3], plate_force[4], plate_force[5]), false);
            ...
            if (t>particle_moving && counter % 500 == 0) {
